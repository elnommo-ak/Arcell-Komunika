...
const db = new sqlite3.Database('./database.db', (err) => {
    if (err) {
        ...
        db.run("PRAGMA foreign_keys = ON;", (pragmaErr) => {
            ...
            initTables();
            initScheduler(); // Jalankan cron job setelah database siap
        });
    }
});
...
function initTables() {
    db.serialize(() => {
        ...
        // 6. Tabel Scheduled Transactions (Fitur Transaksi Terjadwal)
        ...
        db.all(
            ...
            async (err, rows) => {
                ...
                for (const item of rows) {
                    // Lock status agar tidak dieksekusi ganda
                    ...
                    // Gunakan IP 127.0.0.1 lokal server untuk menghindari error resolution DNS
...
async function kirimPushNotif(pesanTitle, pesanBody, targetId = null, isExternalId = false) {
    try {
        ...
        const payload = {
            ...
            // ⚡ MENCEGAH MENIMPA NOTIFIKASI LAMA:
            collapse_id: uniqueNotificationId,    // Mengatur ID unik untuk Web Push
            web_push_topic: uniqueNotificationId  // Mencegah browser mengganti banner notifikasi sebelumnya
        };
        ...
        if (cleanTargetId) {
            ...
            // Jika targetId kosong/invalid, fallback broadcast agar tidak No Recipients
...
app.post('/api/user/schedule-transaction', (req, res) => {
    ...
    // Format schedule_time dipastikan ke bentuk YYYY-MM-DD HH:mm:ss
...
// DELETE: Hapus / Batal transaksi terjadwal
